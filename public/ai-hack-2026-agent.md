...
- **リポジトリ**：https://github.com/0k4c/ai-hack-2026 （現在は非公開。本文中のIssue・PR・測定記録へのリンクも、閲覧権限があるメンバー向けです）
- **チーム**：3人（全員ハッカソン初参加）
- **開発期間**：2026/9/19 〜 9/22
...
手配1件の記録を、1枚の画面で確認します。リポジトリを取得できるメンバーは、`node app/serve.mjs` を起動して `http://127.0.0.1:4173` を開き、「架空サンプルを見る」を選ぶと、**APIを呼ばずに同じ画面を試せます**。以下の画面は、表示確認用にあらかじめ用意した固定の架空サンプルです。実行時のAI判断や、後述する通信モックで生成した記録ではありません。
