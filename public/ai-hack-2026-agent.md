...
■ 公開前に残る確認
  [ ] #71（承認統一）・#72（診断）・#74（勤務管理）・#75（画像）の最終マージ状況に合わせて本文を更新
  [ ] #66の最終画像をQiitaへ配置し、下の画像キャプションと組み合わせる。通し録画は別途未作成
  [ ] #17の人の品質所見・Console設定・設定後計測が届けば追記。届かなければ未実施の説明を残す
  [ ] リポジトリの公開／ソース提出方法を3人で決め、記事の参照リンクを第三者が開けるか確認（#65）
  [ ] 記事を人が最終確認して公開し、Qiita上の表示と記事URLを確認（#16）

比較18回、既存の通し成功・失敗例、モックの説明は本文へ反映済み。
公開・Issueの完了判定・他PRのマージはこの下書き更新では行わない。
...
<!--
画像差し込み待ち（#66）。出典とキャプションは「動いているところ」のコメントを使用。
現在PrivateのGitHub画像URLをQiitaの公開画像URLと扱わない。
-->
...
<!--
画像出典：PR #75、commit 6c02856cf617ab81cfcf0821b7f1bf1ed0b91417、app/demo-assets/。
PR #71を土台にPR #75で撮影したレビュー画像。マージ後の最終画像を確認し、Qiitaへ配置したURLで埋め込む。
01-overview-mock.png：架空の6人で欠勤対応を再現した画面。AI応答は固定モックで、外部への打診は未送信です。
02-decision-mock.png：コードによる候補判定と、固定モックによる選定理由・辞退の返事を表示しています。実AIの品質を示す画像ではありません。
03-approved-mock.png：モックで作った承認待ち記録を店長ボタンで承認し、ローカルJSONへ保存した状態です。本番の勤務表・勤怠・給与は更新しません。
04-metrics-api-pr21.png：PR #21の2026-09-20の既存実API記録。2巡・2,540トークン・概算0.000358 USD・18.47秒。今回の再計測ではありません。
実APIの費用画像とモックの承認画像を同一実行の連続画像として並べない。
通し録画は未作成。静止画を録画済みと書かない。
-->
