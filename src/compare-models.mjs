...
export function comparisonMarkdown(report) {
  const rows = report.rows.map(row => `| ${row.task} | ${cell(row.requestedModel)} | ${row.repeat} | ${cell(row.actualModel)} | ${row.status} | ${cell(row.httpStatus)} | ${cell(row.error?.code ?? '—')} | ${cell(row.durationMs)} | ${cell(row.tokens.total)} | ${cost(row.estimatedCostUsd)} | 未記入 |`).join('\n');
