import { personName, friendlyText } from './labels.mjs';
...
const paragraph = value => `<p class="report-text">${text(friendlyText(value))}</p>`;

export function validateRecord(record) {
  if (!object(record) || record.schemaVersion !== 1) throw new Error('この記録形式には対応していません。アプリから保存した手配記録を選んでください。');
  if (typeof record.processId !== 'string' || !object(record.vacancy) || !Object.hasOwn(statuses, record.status)
    ...
    throw new Error('手配記録の内容が不足しています。別の記録を選ぶか、管理者に確認してください。');
  }
...
function measurements(value = {}) {
  return `<details class="technical"><summary>AIの処理・費用の詳細</summary><div class="metrics"><span>実モデル <strong>${text(value?.actualModel)}</strong></span><span>トークン <strong>${tokens(value)}</strong></span><span>概算USD <strong>${measuredCost(value)}</strong></span><span>所要時間 <strong>${duration(value?.durationMs)}</strong></span></div>
    <details class="technical"><summary>計測の詳細</summary><dl><dt>要求モデル</dt><dd>${text(value?.requestedModel)}</dd><dt>入力 / 出力</dt><dd>${unavailable(value) ? '不明 / 不明' : `${number(value?.tokens?.prompt)} / ${number(value?.tokens?.completion)}`}</dd><dt>API呼び出し</dt><dd>${value?.apiCalled === true ? 'あり' : value?.apiCalled === false ? 'なし' : '不明'}</dd><dt>費用の取得元</dt><dd>${text(value?.costSource)}</dd><dt>リクエストID</dt><dd>${text(value?.requestId)}</dd></dl></details></details>`;
}
...
function renderRound(round, maxRounds) {
  ...
  const delivery = round.channel === 'dryrun' ? '未送信（デモ）' : round.delivered === false ? '未送信' : round.delivered === true ? '送信済み（記録上）' : '送信状態不明';
...
export function renderRecord(input, { sample = false, review = null } = {}) {
  ...
  return `<section class="intro"><div><div class="muted ${sample ? 'sample-tag' : ''}">${sample ? '架空サンプル / 数値は表示確認用です' : record.dataSource === 'synthetic_seed' ? '架空の従業員データを使った手配記録' : '読み込んだ手配記録'}</div><h1>${time(record.vacancy)} の欠員</h1><p>欠勤者：${text(person(record.vacancy.absentEmployeeId))}</p><details class="technical"><summary>管理用の記録情報</summary><p class="meta">処理ID ${text(record.processId)}<br>記録日時 ${text(record.createdAt)}</p></details></div>${badge(statusLabel, record.status)}</section>
    <details class="technical"><summary>処理時間と費用</summary><section class="summary-strip" aria-label="手配の合計"><div><span class="metric-label">打診回数 / 上限</span><span class="metric-value">${rounds.length} / ${number(record.limits.maxRounds)} 巡</span></div><div><span class="metric-label">合計トークン（初回選定を含む）</span><span class="metric-value">${totalTokens}</span></div><div><span class="metric-label">合計概算費用（USD）</span><span class="metric-value">${totalCost}</span></div><div><span class="metric-label">処理全体の所要時間</span><span class="metric-value">${duration(record.totalDurationMs)}</span></div></section></details>
    ${record.status === 'pending' ? '<p class="muted">処理途中、または中断された記録です。計数・費用は途中値で、未反映の通信がある可能性があります。一覧を更新して最新の記録を開いてください。</p>' : ''}
    <div class="layout"><aside class="panel"><h2>勤務条件に合う候補</h2><p class="muted">${evaluations.length}人を確認し、${candidates}人が候補。<br>対象週の開始：${text(record.filtering.weekStart)}</p><div class="candidate-list">${evaluations.map(entry => `<div class="candidate"><div class="candidate-heading"><span class="candidate-name">${text(person(entry.employeeId))}</span><span class="muted">${entry.eligible === true ? '候補' : entry.eligible === false ? '除外' : '判定不明'}</span></div>${entry.reasons.map(reason => `<p>${text(exclusions[reason] ?? "勤務条件の確認が必要")}</p>`).join('')}<p class="muted">週 ${number(entry.weeklyHoursBefore)} → ${number(entry.weeklyHoursAfter)}h / 上限 ${number(entry.maxWeeklyHours)}h<br>追加後 ${number(entry.consecutiveDaysAfter)}連勤 / 上限 ${number(entry.maxConsecutiveDays)}日</p></div>`).join('') || '<p>候補判定の記録がありません。</p>'}</div></aside>
